...
export default async function PublicHeader({ active }: PublicHeaderProps) {
  ...
  return <header className="bg-[#52604a] text-white">
    <div className="mx-auto flex max-w-6xl flex-wrap items-center justify-between gap-x-5 gap-y-2 px-5 py-3 md:px-8 md:py-4">
      <Link href="/" className="font-serif text-xl focus:outline-none focus:ring-2 focus:ring-white/80 md:text-2xl">{brand.name}</Link>
      <a className="rounded px-1 py-1 text-sm focus:outline-none focus:ring-2 focus:ring-white/80 md:order-3 md:text-base" href={contact.whatsappUrl} target="_blank" rel="noopener noreferrer">WhatsApp ↗</a>
