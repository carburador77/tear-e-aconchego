...
type BatchItem = { id: string; file: File; previewUrl: string; name: string; categoryId: string; price: string; allowDuplicate: boolean; status: ItemStatus; error: string; warning: string; slug?: string };
...
export default function BatchProductsPage() {
  ...
  const chooseFiles = (files: FileList | null) => {
    ...
    Array.from(files).forEach((file) => {
      ...
      idCounter.current += 1;
      additions.push({ id: `${Date.now()}-${idCounter.current}`, file, previewUrl, name: productNameFromFile(file.name), categoryId: defaultCategory, price: '', allowDuplicate: false, status: 'ready', error: '', warning: '' });
    });
  ...
  const registerAll = async () => {
    ...
    for (let index = 0; index < candidates.length; index += 1) {
      ...
      try {
        ...
        const { data, error } = await supabase.from('products').insert({ category_id: item.categoryId, name: item.name.trim(), slug, description: '', price: priceLabel ? null : parsedPrice, image_url: uploadedImage, origin: null, dimensions: null, care: null, whatsapp_url: null, display_order: nextDisplayOrder, active: true }).select('id').single();
        if (error) throw error;
  ...
  return <main className="mx-auto max-w-6xl p-4 sm:p-6">
    ...
    {items.length > 0 && <section className="mt-6">
      ...
      <div className="space-y-3">{items.map((item, index) => { const duplicate = duplicateReason(item, index); return <article key={item.id} className={`grid gap-3 rounded-lg border p-3 sm:grid-cols-[96px_1fr_1fr_130px_auto] ${item.status === 'success' ? 'border-green-600 bg-green-50' : item.status === 'error' ? 'border-red-500 bg-red-50' : 'border-[#bfb3a3] bg-white'}`}>
        ...
        <label className="text-xs font-semibold">Categoria<select value={item.categoryId} disabled={processing || item.status === 'success'} onChange={(event) => updateItem(item.id, { categoryId: event.target.value })}>{categories.map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}</select></label>
